...
class TestAliasNormalizerChain:
    ...
    def test_alias_target_flag_set_at_creation(self, tmp_path):
        ...
        tokenizer.json is the authoritative record of the added tokens. The old
        assertion also read the config's added_tokens_decoder mirror, but
        transformers 5.x no longer writes that field when a tokenizer.json is
        present, so the flag is only checked there.
        """
